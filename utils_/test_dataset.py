...
def valDataLoad(args):
    data_paths = [os.path.join(args.val_data_path, d.name) for d in os.scandir(args.val_data_path)]
    datasets = []
...
class validDataloader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.data_path) as data:
            ...
            img = np.asarray(data['images'][img_k1])[None]/255.0
            max_len = i1 - i0
    ...
    def process_ev(self, events, max_len):
        ...
        if max_len <= next_idx:
            # if not enough events, generate some (between 10 and 100) random events
...
class testDataloader(Dataset):
    def __init__(self, args, data_path):
        ...
        with h5py.File(self.data_path) as f:
            ...
            img_k.sort()
            if len(f['images'][img_k[0]].shape) ==3:
                self.h, self.w = f.attrs['sensor_resolution']
            else:
            ...
            for k in img_k[:-1]:
                ...
                # img_ts = f['images'][k].attrs['timestamp']
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.data_path) as data:
            ...
            i0 = data['images'][img_k0].attrs['event_idx']
            i1 = data['images'][img_k1].attrs['event_idx']
            if len(data['images'][img_k1].shape) == 3:
                img = np.asarray(data['images'][img_k1]).mean(2)[None]/255.0
                # img = np.asarray(data['images'][img_k1]).transpose(2, 0, 1)/255.0
            else:
                img = np.asarray(data['images'][img_k1])[None]/255.0
            if img.shape[1] == 3 and len(img.shape) == 4:
                img = img.mean(1)
            max_len = i1 - i0
